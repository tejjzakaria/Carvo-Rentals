...
export default function EditDamagePage({ basePath, HeaderComponent }: SharedPageProps) {
    ...
    try {
      ...
      if (data.success && data.damage) {
        ...
        setFormData({
          ...
          repairCost: damage.repairCost.toString(),
          insuranceClaim: damage.insuranceClaim,
          claimAmount: damage.claimAmount ? damage.claimAmount.toString() : '',
          status: damage.status,
          images: damage.images || [],
          reportedDate: damage.reportedDate.split('T')[0],
        ...
        setTimeout(() => router.push(`${basePath}/damages`), 2000)
      }
    } catch (error) {
      console.error('Error fetching damage:', error)
      setToast({
        show: true,
        message: 'Failed to load damage',
        type: 'error'
      })
    } finally {
      setFetching(false)
    ...
    try {
      ...
      const response = await fetch(`/api/damages/${damageId}`, {
        ...
        body: JSON.stringify({
          ...
          repairedDate: formData.repairedDate || null
        })
      })
      ...
      if (response.ok && data.success) {
        ...
        setToast({
          show: true,
          message: data.error || 'Failed to update damage',
          type: 'error'
        })
      }
    } catch (error) {
      ...
      setToast({
        show: true,
        message: 'An error occurred while updating damage',
        type: 'error'
      })
    } finally {
      setLoading(false)
  ...
  if (fetching) {
    return (
      <>
        <HeaderComponent title="Edit Damage" subtitle="Loading..." />
        <div className='flex items-center justify-center bg-[#F5F5F5] min-h-screen'>
          <div className='text-center'>
            <div className='inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-primary'></div>
            <p className='text-gray-300 mt-4'>Loading damage...</p>
          </div>
        </div>
      </>
    )
  ...
  return (
    <>
      <HeaderComponent title="Edit Damage" subtitle="Update damage information" />
              ...
              <div className='lg:col-span-2 space-y-6'>
                ...
                <div className='bg-white rounded-2xl shadow-sm border border-gray-200 p-6'>
                  ...
                  <div className='space-y-4'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-gray-500 mt-1'>Rental link cannot be changed</p>
                    </div>
                  </div>
                </div>
                ...
                <div className='bg-white rounded-2xl shadow-sm border border-gray-200 p-6'>
                  ...
                  <div className='space-y-4'>
                    <div>
                      <label className='block text-sm font-semibold text-[#000000] mb-2'>
                        Repair Cost ({settings?.currency || 'USD'})
                      </label>
                      <input
                        type='number'
                        name='repairCost'
              ...
              <div className='lg:col-span-1'>
                <div className='bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sticky top-8'>
                  ...
                  <div className='space-y-3'>
                    ...
                    >
                      {loading ? 'Updating Damage...' : 'Update Damage'}
                    </button>
